...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        const fetchPendingRequests = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/leaving-forms/pending-requests');
    ...
    const handleStatusUpdate = async (id, status) => {
        try {
            const response = await axios.patch(`http://localhost:5000/api/leaving-forms/update-status/${id}`, { status });
