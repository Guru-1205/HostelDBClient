...
const LeavingFormDashBoard = () => {
    ...
    useEffect(() => {
        const fetchPendingRequests = async () => {
            try {
                const response = await axios.get('http://localhost:3031/api/leaving-forms/pending-requests');
    ...
    const handleUpdateStatus = async (id, status) => {
        try {
            await axios.patch(`http://localhost:3031/api/leaving-forms/update-status/${id}`, { status });
