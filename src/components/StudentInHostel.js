...
const StudentInHostel = () => {
    ...
    useEffect(() => {
        const fetchAbsentees = async () => {
            try {
                const response = await axios.get('http://localhost:3031/api/admin/hostel-absentees');
    ...
    const handleReturnToHostel = async (studentId) => {
        try {
            await axios.patch(`http://localhost:3031/api/admin/update-hostel-status/${studentId}`);
